...
class haichart(object):
    ...
    def to_multiple_htmls(self, dict_sorted, tname):
        """
        ...
        self.output('multiple_htmls', dict_sorted, tname)
    ...
    def output(self, output_method, dict_sorted, tname):
        ...
        if output_method == 'single_html':  
            ...
            for i in range(instance.view_num):
                ...
                old_view = view
            return export_dict
        elif output_method == 'multiple_htmls':
            path3 = os.getcwd() + '/html/' + tname
            ...
            for list in dict_sorted:
                ...
                if old_view:
                    ...
                    order1 += 1
                old_view = view
                self.html_output(order1, view, 'multiple', tname, num)
                num = num + 1
    ...
    def html_output(self, order, view, mode, tname, num):
        ...
        if mode == 'single':
            ...
            grid.render('./html/' + tname + '/' + str(order) + '.html')  # the grid is added in a new file
            if not os.path.exists('./html/' + tname + '/' + str(order) + '.png'):
                make_snapshot(snapshot, grid.render(), './html/' + tname + '/' + str(order) + '.png')
    # Drawing code
