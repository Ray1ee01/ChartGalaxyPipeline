...
function makeChart(containerSelector, data) {
    ...
    // 当x维度数量大于15时，动态增加svg高度
    if (dimensions.length > 15) {
        const extraDimensions = dimensions.length - 15;
        const heightIncrease = height * (extraDimensions * 0.03); // 每增加一个维度，高度增加3%
        height += heightIncrease;
        console.log(`X维度数量: ${dimensions.length}, 超出15个的数量: ${extraDimensions}`);
        console.log(`原始高度: ${variables.height || 600}, 增加高度: ${heightIncrease}, 最终高度: ${height}`);
    }
